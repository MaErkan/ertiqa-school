...
import {
  FileCheck, Users, BookOpen, TrendingUp, Plus, Eye, FileText
} from 'lucide-react';
import {
...
import { subjects, monthlyVisitData, subjectPerformanceData, visitDistributionData } from '@/data/demoData';

const kpiCards = [
  { icon: FileCheck, label: 'إجمالي الزيارات', value: '24', sub: 'هذا الشهر', color: '#8A1538', change: '+12%' },
  { icon: Users, label: 'المعلمين', value: '67', sub: 'معلم نشط', color: '#4A7FB5', change: '+3' },
  { icon: BookOpen, label: 'المواد', value: '12', sub: 'مادة دراسية', color: '#2E7D5A', change: '' },
  { icon: TrendingUp, label: 'متوسط الأداء', value: '4.2', sub: 'من 5 نجوم', color: '#C9892E', change: '+0.3' },
];
  ...
  return (
    <DashboardLayout pageTitle="لوحة التحكم">
        ...
        {kpiCards.map((card, i) => (
          ...
          >
            ...
            <div className="flex items-start justify-between mb-3">
              ...
              {card.change && (
                <span className="badge badge-success text-xs">{card.change}</span>
              )}
            </div>
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={monthlyVisitData}>
              ...
              <Tooltip
                contentStyle={{ fontFamily: 'Tajawal', borderRadius: 12, border: '1px solid #E6DCD4' }}
                formatter={(value: number) => [`${value}`, 'الزيارات']}
              />
          ...
          <ResponsiveContainer width="100%" height={240}>
            <PieChart>
              <Pie
                ...
                nameKey="name"
              >
                {visitDistributionData.map((entry, i) => (
              ...
              <tbody>
                {recentVisits.map((visit) => (
                  <tr key={visit.id}>
                    ...
                    <td>
                      <span className="badge badge-maroon text-xs">{visit.subjectName}</span>
                    </td>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {subjects.map(s => (
            ...
            >
              ...
              <p className="font-ibm text-xs" style={{ color: 'var(--gray-500)' }}>{s.teachers.length} معلم</p>
            </button>
          ))}
        </div>
      </motion.div>
      ...
      <button
        ...
        className="fixed bottom-8 left-8 w-14 h-14 rounded-full flex items-center justify-center shadow-lg z-50 transition-transform hover:scale-110 animate-pulse-gold"
